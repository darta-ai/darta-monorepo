...
import {ScrollView, View, Image, Keyboard, Animated} from 'react-native';
...
export function UserSettingsSignedIn({uniqueId}: {uniqueId: string}) {
  ...
  const handleButtonClick = async (formName: ButtonTypes) => {
    ...
    switch (formName) {
      case ButtonTypes.profilePicture:
        ...
        break;
      default:
        const isEditing = formData[formName].isEditing;
        if (!isEditing) {
          handleShrinkElements();
  ...
  const onSave = async (data: unknown) => {
    ...
    dispatch({type: ETypes.setUserSettings, userSettings: {
        ...getValues()
    }})
    resetUi();
  };
  ...
  return (
    <>
      <ScrollView style={{height: hp('100%')}}>
        ...
        <Animated.View
          style={!formData.email.isEditing && {height: heightAnim}}>
          <View style={settingsStyles.divider} />
          <View
            style={
              !formData.email.isEditing && showOnlyOne && {display: 'none'}
            }>
        ...
        <Animated.View
          style={!formData.phone.isEditing && {height: heightAnim}}>
          <View style={settingsStyles.divider} />
          <View
            style={
              !formData.phone.isEditing && showOnlyOne && {display: 'none'}
            }>
