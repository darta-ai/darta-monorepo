...
const calculateAngularFOV = (horizontalDimensionOfSensor: number, focalLength: number): number =>{
    return 2 * Math.tan(1)**-1 * (horizontalDimensionOfSensor / (2*focalLength))
    //where h is the horizontal dimension of the sensor and F is the focal length of the camera lens.
}

const calculateFOV = (horizontalDimensionOfSensor: number, focalLength: number): number =>{
    return 2 * Math.tan(1)**-1 * (horizontalDimensionOfSensor / 2*focalLength)
    //where h is the horizontal dimension of the sensor and F is the focal length of the camera lens.
}

const calculateDiameterFOV = (fov: number, magnification:number): number => {
    return (fov / magnification )
}
