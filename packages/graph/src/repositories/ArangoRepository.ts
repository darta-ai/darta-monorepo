...
> implements IArangoCollection<T, D>
{
  ...
  constructor(
    ...
    this.schema = schema;
    this.fullSchema = schema.concat(
      this.isEdgeCollection ? edgeSchema : documentSchema,
    );
  }
  ...
  protected validate(
    data: D | (D & T) | NotEmptyObject,
    isCreate: boolean = false,
  ) {
    const validation = (isCreate ? this.schema : this.fullSchema).validate(
      data,
    );
  ...
  async create(data: D): Promise<D & T> {
    ...
    if (this.createQry) {
      return this.createQry(data, this.collection.save);
    }
    try {
  ...
  async update(data: NotEmptyObject & T): Promise<D & T> {
    this.validate(data);
    if (this.updateQry) return this.updateQry(data, this.collection.update);
    const {new: newDoc} = await this.collection.update(data._key, data, {
      returnNew: true,
    });
    return newDoc as D & T;
  }
  ...
  async delete(id: string): Promise<string> {
    if (this.deleteQry) return this.deleteQry(id, this.collection.remove);
...
@injectable()
export abstract class DocumentRepository<D> extends ArangoRepository<
  DocumentCollection,
  ArangoDocumentType,
  D
> {
...
@injectable()
export abstract class EdgeRepository<D> extends ArangoRepository<
  EdgeCollection,
  ArangoEdgeType,
  D
> {
