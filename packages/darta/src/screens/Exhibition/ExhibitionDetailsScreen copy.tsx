...
import {View, StyleSheet, ScrollView, Platform, Linking, RefreshControl, Text, Alert, TouchableOpacity } from 'react-native';
...
export function ExhibitionDetailsScreen({
  ...
  const isGroupShow = React.useMemo(() => currentExhibition?.exhibitionType?.value === "Group Show", [currentExhibition]);
  const [artistName, setArtistName] = React.useState<string>("")

  const setExhibitionData = React.useCallback(({currentExhibition, currentGallery} : {currentExhibition: Exhibition, currentGallery: IGalleryProfileData}) => {
    ...
    const fetchExhibitionById =  React.useCallback(async (): Promise<{exhibition: Exhibition, galleryData: IGalleryProfileData} | void> => {
 ...
 const setViewedInDB = async ({exhibitionId} : {exhibitionId: string}): Promise<void> => {
    ...
    async function handleExhibitionData() {
        ...
        if (exhibitionId && 
            ...
            galleryState.galleryData[galleryId]){
                ...
                const exhibition = await setExhibitionDataInState()
                setViewedInDB({exhibitionId})
                if (exhibition){
                    uiDispatch({
                        type: UiETypes.setCurrentHeader,
  ...
  const onRefresh = async () => {
    ...
    React.useEffect(() =>{
      ...
      }, [exhibitionId, exhibitionState.userSavedExhibitions, exhibitionState.userSavedExhibitions[exhibitionId]])
    ...
    const rateExhibitionAlert = React.useCallback(() => {
        if (currentExhibition?.exhibitionTitle?.value){
    ...
    const removeExhibitionFromList = React.useCallback(async (rating?: string) => { 
        try{
            setLoadingFollow(true)
            Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                        ...
                        {!addedExhibition && isOpenExhibition && (
                            <View>
                                <TouchableOpacity style={{...exhibitionDetailsStyles.pressableStyle, backgroundColor: Colors.PRIMARY_50, borderColor: Colors.PRIMARY_500, borderWidth: 1}} onPress={() => addExhibitionToList()}>
                                    ...
                                    <TextElement style={{...globalTextStyles.boldTitleText, color: Colors.PRIMARY_950}}>Add To Your List</TextElement>
                                    </View>
                                </TouchableOpacity>
                            </View>
                            )}
                        {addedExhibition && isOpenExhibition && (
                            <View>
                                ...
                                {loadingFollow && <ActivityIndicator size={20} color={Colors.PRIMARY_50} />}
                                    ...
                                    <View style={{width: 170, alignSelf: 'center'}}>
                                    <TextElement style={{...globalTextStyles.boldTitleText, color: Colors.PRIMARY_50}}>Added To Your List</TextElement>
                                    </View>
                                </TouchableOpacity>
                            </View>
                        )} 
